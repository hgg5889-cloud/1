...
def plot_graph(ax, df, predicted_price, levels=None, trend_outlook=None, prediction_text=None):
    ...
    ax.plot(view["close"], label="实际价格", color="#38bdf8", linewidth=1.8)
    ax.axvline(x=len(view), color="r", linestyle="--", label="预测点")
    ax.scatter(len(view), predicted_price, color="#22c55e", label="预测价格", zorder=5)
    if levels:
        x_pos = len(view) - 1
    ...
    if trend_outlook:
        ax.text(
            ...
            bbox=dict(boxstyle="round", facecolor="#111827", alpha=0.8, edgecolor="#1f2937"),
            color="#e5e7eb",
        )
    if prediction_text:
        ax.text(
    ...
    ax.grid(True, color="#1f2937", alpha=0.6, linestyle="--", linewidth=0.6)
    ax.legend(loc="best", frameon=False, labelcolor="#e5e7eb")
    ax.set_title("BTC价格与预测结果", color="#e5e7eb")
...
def run_gui():
    ...
    style.configure("Card.TFrame", background=accent_bg, relief="flat")
    style.configure("TLabel", background=base_bg, foreground=text_primary, font=("Segoe UI", 10))
    style.configure("Heading.TLabel", font=("Segoe UI Semibold", 11), foreground=highlight, background=base_bg)
    style.configure("Title.TLabel", font=("Segoe UI Semibold", 14), foreground=highlight, background=base_bg)
    style.configure(
        "TButton",
        font=("Segoe UI", 10),
        padding=(8, 4),
    ...
    style.configure("Treeview.Heading", background=accent_bg, foreground=text_primary, font=("Segoe UI Semibold", 10))
    style.map(
        "Treeview",
    ...
    header_frame.pack(fill=BOTH, padx=16, pady=(16, 4))
    Label(header_frame, text="BTC量化交易工具", style="Title.TLabel").pack(side=LEFT)
    Label(header_frame, text="高级量化面板", foreground=text_muted).pack(side=LEFT, padx=12)

    control_frame = Frame(root)
    control_frame.pack(fill=BOTH, padx=16, pady=8)

    Label(control_frame, text="数据周期:", style="Heading.TLabel").pack(side=LEFT)
    ...
    Label(control_frame, text="刷新(秒):", style="Heading.TLabel").pack(side=LEFT)
    ...
    Label(control_frame, text="模型:", style="Heading.TLabel").pack(side=LEFT, padx=6)
    ...
    Label(control_frame, text="支撑/压力版本:", style="Heading.TLabel").pack(side=LEFT, padx=6)
    ...
    Label(control_frame, text="宏观指标:", style="Heading.TLabel").pack(side=LEFT, padx=6)
    macro_listbox = Listbox(control_frame, selectmode="multiple", height=3, exportselection=False)
    macro_listbox.configure(bg=accent_bg, fg=text_primary, selectbackground="#2563eb", selectforeground="#f9fafb")
    ...
    Label(control_frame, text="资金流向:", style="Heading.TLabel").pack(side=LEFT, padx=6)
    flow_listbox = Listbox(control_frame, selectmode="multiple", height=3, exportselection=False)
    flow_listbox.configure(bg=accent_bg, fg=text_primary, selectbackground="#2563eb", selectforeground="#f9fafb")
    for label, _ in flow_sources:
        flow_listbox.insert(END, label)
    flow_listbox.pack(side=LEFT, padx=6)
    flow_listbox.select_set(0, END)

    status_label = Label(control_frame, text="状态: 手动刷新", foreground=text_muted)
    status_label.pack(side=RIGHT)

    strategy_config_frame = Frame(root)
    strategy_config_frame.pack(fill=BOTH, padx=16, pady=6)
    Label(strategy_config_frame, text="策略定制", style="Heading.TLabel").pack(side=LEFT, padx=6)
    ...
    metrics_frame.pack(fill=BOTH, padx=16, pady=8)
    Label(metrics_frame, text="关键指标", style="Heading.TLabel").pack(side=LEFT, padx=6)
    ...
    report_frame.pack(fill=BOTH, padx=16, pady=8)
    Label(report_frame, text="分类输出", style="Heading.TLabel").pack(side=LEFT, padx=6)
    ...
    strategy_frame.pack(fill=BOTH, padx=16, pady=8)
    Label(strategy_frame, text="策略表", style="Heading.TLabel").pack(side=LEFT, padx=6)
    ...
    account_frame.pack(fill=BOTH, padx=16, pady=8)
    Label(account_frame, text="币安模拟仓", style="Heading.TLabel").pack(side=LEFT, padx=6)
